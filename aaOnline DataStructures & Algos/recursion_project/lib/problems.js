...
function sumArray(array, startIdx = 0) {
    if (array[startIdx] === undefined) return 0;
    return array[startIdx] + sumArray(array, startIdx + 1);
}
...
function reverseString(str, ans = "", startIdx = str.length - 1) {
    if(startIdx < 0) return ans;
    ans += str[startIdx]
    return reverseString(str, ans, startIdx - 1);
}
...
function pathFinder(directories, targetFile) {
    if (!fileFinder(directories, targetFile)) return null;
    const keys = Object.keys(directories)
    let ans = '';
    keys.forEach(key => {
        if (directories[key] && pathFinder(directories[key], targetFile)) {
            ans = key + pathFinder(directories[key], targetFile);
        }else {
            if (key === targetFile) {
                ans = '/' + key;
            }
